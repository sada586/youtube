...
exports.run = async (bot, message, args) => {
  ...
  if (!args[0]) return message.channel.send('*Sa-as yazısını açmak için `t!sa-as aç veya kapat`**')
  ...
  if (args[0] == 'kapat') {
    db.set(`saas_${message.guild.id}`, 'kapali')
      ...
      message.channel.send(`**Başarıyla \`sa-as Sistemini\` Kapattınız, Artık Bot \`sa\` Yazıldığında Cevap Vermeyecek.**`)
