...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("» Relax-bot Destek Sistemi")
        .setDescription("**Buradaki destek ekibimiz sizinle ilgilenecektir.\nDestek talebini iptal etmek için [t!kapat](sunuculinkvardı ) yazabilirsin!**")
        .setFooter('Relax-bot | Destek Sistemi', client.user.avatarURL)
        .setTimestamp()
        ch.send(embed).catch()
        
        ch.send("@here")
        ch.awaitMessages((msg)=> {
            if (msg.content === "t!kapat") {
