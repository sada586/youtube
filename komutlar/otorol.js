...
exports.run = async(client, message, args) => {
    var bymayfe = args[0];
  if(!bymayfe) return message.channel.send("Bir seçenek belirtin (ayarla - kapat).**Doğru Kullanım: t!otorol ayarla @Uye #Kanal**");
 ...
 let rol = message.mentions.roles.first() || message.guild.roles.get(args.join(' '))
  let newRole;
  let tworole;
  if (!rol) return message.channel.send(':no_entry: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `t!otorol ayarla @Rol #kanal`')
  ...
  if (!otorolkanal) return message.channel.send(':no_entry: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `t!otorol ayarla @Rol #kanal`')
